...
# Admin View for Active Bets
class ActiveBetAdminView(discord.ui.View):
    """View with admin controls for active bets"""
    ...
    def __init__(self, bet_id: int, bet_title: str, options: list):
        ...
        resolve_button = discord.ui.Button(
            ...
            custom_id=f"resolve_bet_{bet_id}",
            row=0
        )
        resolve_button.callback = self.resolve_bet_callback
        ...
        lock_button = discord.ui.Button(
            ...
            custom_id=f"lock_bet_{bet_id}",
            row=0
        )
        lock_button.callback = self.lock_bet_callback
        ...
        cancel_button.callback = self.cancel_bet_callback
        self.add_item(cancel_button)
    
    async def resolve_bet_callback(self, interaction: discord.Interaction):
        ...
        # Import BetResolutionView from betting cog
        try:
            from cogs.betting import BetResolutionView
            view = BetResolutionView(self.bet_id, self.bet_title, self.options)
            ...
            embed.add_field(name="Options", value="\n".join([f"• {opt}" for opt in self.options]), inline=False)
            embed.set_footer(text="Select the winning option below")
            
            await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
        except Exception as e:
            logger.error(f"Error showing resolution view: {e}")
            await interaction.response.send_message("❌ Failed to show resolution options.", ephemeral=True)
    
    async def lock_bet_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if cursor.rowcount > 0:
                # Update the embed to show locked status
                if interaction.message and interaction.message.embeds:
                    embed = interaction.message.embeds[0]
                    # Update status field
                    for i, field in enumerate(embed.fields):
                        if field.name == "Status":
                            embed.set_field_at(i, name="Status", value="🔒 Locked", inline=True)
                            break
                    embed.color = discord.Color.orange()
                    await interaction.message.edit(embed=embed)
                
                await interaction.response.send_message(
                    f"🔒 Bet #{self.bet_id} has been locked. No more bets can be placed.",
                    ephemeral=True
                )
                ...
                try:
                    ...
                    if channels_cog:
                        bet = await self.get_bet_data(self.bet_id)
                        if bet:
                            await channels_cog.update_active_bet_status(bet, 'locked')
                except Exception as e:
                    logger.error(f"Error posting lock status: {e}")
            else:
    ...
    async def cancel_bet_callback(self, interaction: discord.Interaction):
        ...
        try:
            from cogs.betting import BetCancelConfirmModal
            modal = BetCancelConfirmModal(self.bet_id, self.bet_title)
            await interaction.response.send_modal(modal)
        except Exception as e:
            logger.error(f"Error showing cancel modal: {e}")
            await interaction.response.send_message("❌ Failed to show cancel confirmation.", ephemeral=True)
    
    async def get_bet_data(self, bet_id: int):
...
class Channels(commands.Cog):
    ...
    async def post_bet_creation(self, bet_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Create admin view with buttons
            admin_view = ActiveBetAdminView(bet_data["bet_id"], bet_data["title"], options)
            
            message = await channel.send(embed=embed, view=admin_view)
